...
class TradeState:
    ...
    def update_trailing_sl(self, current_price: float) -> None:
        """Update trailing SL based on current P&L per lot"""
        pnl = self.calc_pnl_per_lot(current_price)
        if pnl > self.highest_pnl_per_lot:
            self.highest_pnl_per_lot = pnl

        step = self.tsl_step_per_lot
        if self.highest_pnl_per_lot >= step:
            # Number of complete steps achieved
            steps_achieved = int(self.highest_pnl_per_lot / step)
            # SL = (steps_achieved - 1) * step
            # e.g., profit hits 500 → SL at 0 (cost), profit hits 1000 → SL at 500
            new_sl = (steps_achieved - 1) * step
    ...
    def should_exit(self, current_price: float) -> Optional[str]:
        ...
        # Target hit
        if pnl >= self.target_per_lot:
            return f"TARGET (₹{pnl:.0f}/lot)"
...
class FuturesRuntime:
    ...
    direction_locked: Optional[str] = None  # "BUY" or "SELL" or None
...
class LiveLTPFeed:
    ...
    def __init__(self, access_token: str, on_tick=None) -> None:
        ...
        self._on_tick_callback = on_tick  # Called with (symbol, ltp) on every tick
...
class ORBStrategyV2:
    ...
    def initialize(self) -> bool:
        ...
        print(f"[INIT]   Futures — Target: ₹{self.cfg.futures_target:.0f}, SL: ₹{self.cfg.futures_sl:.0f}, TSL Step: ₹{self.cfg.futures_tsl_step:.0f}")
        print(f"[INIT]   Options — Target: ₹{self.cfg.options_target:.0f}, SL: ₹{self.cfg.options_sl:.0f}, TSL Step: ₹{self.cfg.options_tsl_step:.0f}")
    ...
    def _fetch_opening_ranges(self) -> None:
        ...
        start = now_ist().replace(hour=orb_h, minute=orb_m, second=0, microsecond=0)
        now = now_ist()

        # Fetch ALL candles from 09:15 to now (first candle = ORB, rest = RSI warmup)
        print(f"\n[HISTORY] Loading candles from {start.strftime('%H:%M')} to {now.strftime('%H:%M')} for RSI warmup...")
        ...
        for name, rt in self.futures_runtimes.items():
            try:
                ...
                if candles:
                    # First candle = ORB
                    c = candles[0]
                    rt.opening_range = OpeningRange(high=c.high, low=c.low)
                    # Load ALL candles for RSI calculation
                    rt.candles = candles
                    rsi = calc_rsi(rt.candles, self.cfg.rsi_period)
                    rsi_str = f"{rsi:.1f}" if rsi else "need more candles"
                    print(f"[ORB FUT] {name}: High={c.high:.2f} Low={c.low:.2f} "
                          f"| {len(candles)} candles loaded | RSI={rsi_str}")
                else:
        ...
        for name, rt in self.options_runtimes.items():
            for leg in [rt.ce_leg, rt.pe_leg]:
                ...
                try:
                    ...
                    if candles:
                        c = candles[0]
                        leg.opening_range = OpeningRange(high=c.high, low=c.low)
                        leg.candles = candles
                        rsi = calc_rsi(leg.candles, self.cfg.rsi_period)
                        rsi_str = f"{rsi:.1f}" if rsi else "need more candles"
                        print(f"[ORB OPT] {name} {leg.opt_type}: High={c.high:.2f} Low={c.low:.2f} "
                              f"| {len(candles)} candles loaded | RSI={rsi_str}")
                    else:
    ...
    def _process_all(self) -> None:
        ...
        for name, rt in self.futures_runtimes.items():
            ...
            if rt.opening_range is None:
                try:
                    orb_h, orb_m = self.cfg.orb_candle_start
                    orb_start = now_ist().replace(hour=orb_h, minute=orb_m, second=0, microsecond=0)
                    candles = self.broker.history(rt.futures_symbol, str(tf),
                                                  int(orb_start.timestamp()), int(n.timestamp()))
                    if candles:
                        c = candles[0]
                        rt.opening_range = OpeningRange(high=c.high, low=c.low)
                        rt.candles = candles
                        rsi = calc_rsi(rt.candles, self.cfg.rsi_period)
                        rsi_str = f"{rsi:.1f}" if rsi else "need more candles"
                        print(f"[ORB RETRY ✓] {name} FUT: High={c.high:.2f} Low={c.low:.2f} "
                              f"| {len(candles)} candles loaded | RSI={rsi_str}")
                    else:
                        rt.last_processed_bucket = current_bucket
                        continue
        ...
        try:
            ...
            if rt.current_trade and rt.current_trade.is_live:
                ...
                print(f"[FUT TRADE] {name}: {side_str} LTP={price:.2f} "
                      f"RSI={rsi_text} "
                      f"P&L/lot=₹{pnl:.0f} TSL@₹{rt.current_trade.current_sl_per_lot:.0f}")
                return
            ...
            if rt.entry_state == "WAITING":
                ...
                if (is_green and candle.open < orb.high and candle.close > orb.high
                        and (rt.direction_locked is None or rt.direction_locked == "BUY")):
                ...
                # Bearish: RED candle opens above ORB Low, closes below ORB Low
                        ...
                        and (rt.direction_locked is None or rt.direction_locked == "SELL")):
                    ...
                    # Confirmation: candle closes above breakout_level
                        ...
                        if rsi is not None and rsi > 50:
                            ...
                            print(f"[FUT] {name}: C={candle.close:.2f} > "
                                  ...
                                  f"RSI={rsi_str} [CONFIRMED but RSI<50, waiting...]")
    ...
    def _enter_futures(self, name: str, rt: FuturesRuntime,
        ...
        rt.current_trade = TradeState(
            ...
            tsl_step_per_lot=self.cfg.futures_tsl_step,
            order_id=oid,
        )
            ...
            if not leg.opening_range:
                try:
                    ...
                    if candles:
                        ...
                        rsi = calc_rsi(leg.candles, self.cfg.rsi_period)
                        rsi_str = f"{rsi:.1f}" if rsi else "need more candles"
                        print(f"[ORB RETRY ✓] {name} {leg.opt_type}: High={c.high:.2f} Low={c.low:.2f} "
                              f"| {len(candles)} candles loaded | RSI={rsi_str}")
                    else:
                        continue
                except Exception:
            ...
            try:
                ...
                if leg.current_trade and leg.current_trade.is_live:
                    ...
                    print(f"[OPT TRADE] {name} {leg.opt_type}: SELL Prem={price:.2f} "
                          f"RSI={rsi_text} "
                          f"P&L/lot=₹{pnl:.0f} TSL@₹{leg.current_trade.current_sl_per_lot:.0f}")
                    continue
    ...
    def _enter_option(self, name: str, leg: OptionLeg, price: float) -> None:
        ...
        leg.current_trade = TradeState(
            ...
            tsl_step_per_lot=self.cfg.options_tsl_step,
            order_id=oid,
        )
