...
class StrategyGUI:
    ...
    def _build_left(self, parent) -> None:
        ...
        ctk.CTkSegmentedButton(
            mode_row, values=["PAPER", "LIVE"], variable=self.mode_var,
        ...
        # ── Futures Target/SL ──
        ft = self._section(parent, "FUTURES — Target/SL (₹/lot)")
        self.fut_target_var = ctk.StringVar(value="3000")
        self.fut_sl_var = ctk.StringVar(value="1500")
        self.fut_trail_gap_var = ctk.StringVar(value="100")
        ...
        self._row(ft, "Trail Gap", lambda p: self._entry(p, self.fut_trail_gap_var, 70))

        # ── Options Target/SL ──
        ot = self._section(parent, "OPTIONS — Target/SL (₹/lot)")
        self.opt_target_var = ctk.StringVar(value="2000")
        self.opt_sl_var = ctk.StringVar(value="1000")
        self.opt_trail_gap_var = ctk.StringVar(value="100")
        ...
        self._row(ot, "Trail Gap", lambda p: self._entry(p, self.opt_trail_gap_var, 70))
    ...
    def _build_right(self, parent) -> None:
        ...
        # ── Closed Trades Tab ──
        closed_tab = tabs.add("Closed Trades")
        self._build_closed_trades_tab(closed_tab)
    ...
    def _build_closed_trades_tab(self, parent) -> None:
        self.closed_scroll = ctk.CTkScrollableFrame(parent, fg_color=self.CLR_PANEL)
        self.closed_scroll.pack(fill="both", expand=True)

        # Header
        hdr = ctk.CTkFrame(self.closed_scroll, fg_color=self.CLR_ACCENT, corner_radius=6, height=32)
        hdr.pack(fill="x", padx=4, pady=(4, 2))
        hdr.pack_propagate(False)

        self._closed_headers = ["Time", "Instrument", "Side", "Entry", "Exit", "P&L", "Reason"]
        self._closed_widths = [65, 90, 40, 65, 65, 75, 120]

        for text, w in zip(self._closed_headers, self._closed_widths):
            ctk.CTkLabel(hdr, text=text, width=w, font=ctk.CTkFont(size=10, weight="bold"),
                         text_color="#fff", anchor="center").pack(side="left", padx=1)

        self._closed_row_frames: list = []
    ...
    def _build_config(self) -> StrategyConfig:
        ...
        return StrategyConfig(
            paper_trading=(self.mode_var.get() == "PAPER"),
            ...
            futures_target_trail_gap=safe_float(self.fut_trail_gap_var, 100),
            options_target=safe_float(self.opt_target_var, 2000),
            options_sl=safe_float(self.opt_sl_var, 1000),
            options_target_trail_gap=safe_float(self.opt_trail_gap_var, 100),
            instruments=instruments,
        )

    # ── Mode Toggle ────────────────────────────────────────────────────────
    def _on_mode_change(self, value: str) -> None:
        pass  # direct toggle, no confirmation needed
        ...
        if self.runner and self.runner.engine:
            try:
                ...
                if len(closed) != self._closed_count:
                    # Clear old rows
                    for frm in self._closed_row_frames:
                        frm.destroy()
                    self._closed_row_frames.clear()
                    # Add new rows
                    for i, trade in enumerate(closed):
                        bg = self.CLR_CARD if i % 2 == 0 else self.CLR_PANEL
                        row = ctk.CTkFrame(self.closed_scroll, fg_color=bg, corner_radius=4, height=30)
                        row.pack(fill="x", padx=4, pady=1)
                        row.pack_propagate(False)
                        vals = [trade["time"], trade["instrument"], trade["side"],
                                trade["entry"], trade["exit"],
                                f"₹{trade['pnl']:,.0f}", trade["reason"]]
                        for val, w in zip(vals, self._closed_widths):
                            tc = self.CLR_GREEN if "₹" in str(val) and trade["pnl"] >= 0 else \
                                 self.CLR_RED if "₹" in str(val) and trade["pnl"] < 0 else self.CLR_TEXT
                            ctk.CTkLabel(row, text=val, width=w, font=ctk.CTkFont(size=10),
                                         text_color=tc, anchor="center").pack(side="left", padx=1)
                        self._closed_row_frames.append(row)
                    self._closed_count = len(closed)

            except Exception as e:
                print(f"[GUI ERROR] _poll_pnl: {e}")
    ...
    def _save_settings(self) -> None:
        data = {
            ...
            "fut_target": self.fut_target_var.get(), "fut_sl": self.fut_sl_var.get(), "fut_trail_gap": self.fut_trail_gap_var.get(),
            "opt_target": self.opt_target_var.get(), "opt_sl": self.opt_sl_var.get(), "opt_trail_gap": self.opt_trail_gap_var.get(),
            "instruments": {},
        }
        for name, d in self.inst_vars.items():
    ...
    def _load_settings(self) -> None:
        ...
        try:
            ...
            for var_name, key in [("tf_var", "timeframe"), ("entry_end_h", "entry_end_h"),
                                   ...
                                   ("fut_target_var", "fut_target"), ("fut_sl_var", "fut_sl"), ("fut_trail_gap_var", "fut_trail_gap"),
                                   ("opt_target_var", "opt_target"), ("opt_sl_var", "opt_sl"), ("opt_trail_gap_var", "opt_trail_gap")]:
    ...
    def _on_close(self) -> None:
        ...
        if self.runner and self.runner.is_alive():
            ...
            time.sleep(0.5)
        self.root.destroy()
        os._exit(0)  # Force kill all background threads (WS, watchdog)
